...
export default function Register() {
  ...
  const handleNext = () => {
    if (step === 1) {
      ...
      if (teamName.toLowerCase().includes('vsitr')) return showError("Invalid Name", "Team name must not include the institute's name.");
      setStep(2);
    } else if (step === 2) {
      // Basic check for leader
      if (!leader.name || !leader.email || !leader.phone || !leader.enrollment) {
  ...
  const handleSubmit = async () => {
    ...
    for (let m of allMembers) {
      if (!m.name || !m.gender || !m.enrollment || !m.email || !m.phone) {
        return showError("Missing Info", "Please fill all details for all members.");
      }
      if (!m.email.includes('.ac.in') && !m.email.includes('.edu')) {
        // Just a simple mock validation for college email
        // In real life, it would be a specific domain like @vsitr.ac.in
      }
    }
  ...
  const renderMemberForm = (data, onChange, prefix = "Leader") => (
    <div className={styles.row}>
      ...
      <div className={styles.formGroup}>
        ...
        <input className={styles.input} type="text" maxLength="10" value={data.phone} onChange={e => onChange('phone', e.target.value)} />
      </div>
      <div className={styles.formGroup}>
        <label className={styles.label}>College Email ID</label>
        <input className={styles.input} type="email" value={data.email} onChange={e => onChange('email', e.target.value)} />
      </div>
    </div>
  );
